function [SV_uvw,t,T] = analyticalFirstOrder_amplitudeVW(Av,Aw,phiUV,phiW,L,mu,tf,N,type)
% =======================================================================
%     First Order Analytical Solution to CR3BP Near Colinear Points
%          [Uses Amplitude in V and W motion to define orbit]
...
%
% Format : [SV_uvw,t] = analyticalFirstOrder_amplitudeVW(Av,Aw,...
...
% - Position
u = -Av/k.*cos(s.*t + phiUV);
v = Av.*sin(s.*t + phiUV);
w = Aw.*sin(nu.*t + phiW);
% - Velocity
du = Av/k.*s.*sin(s.*t + phiUV);
dv = Av.*s.*cos(s.*t + phiUV);
