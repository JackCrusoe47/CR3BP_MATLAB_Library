function [SVp,tp,DV] = newton_TwoLevelDiffCorrector_howell(SVp_guess,tp_guess,mu,varargin)
% =======================================================================
...
%          < K.C.Howell and H.J.Pernicka Non-Periodic Version>
% =======================================================================
%
...
%
% Format : [SV0,tf,DV] = newton_TwoLevelDiffCorrector_howell(...
%                        SV0_guess,tp_guess,mu,varargin)
%
...
while IterMax2==0 || iter<IterMax2
    ...
    [SVp,tp] = diffCorr2_step(SVp,tp,mu,M,N);
...
% - Function performs single step of 2nd level diff. corr.
    function [SVp,tp] = diffCorr2_step(SVp,tp,mu,M,N)
...
% Function performs single step of 2nd level on single point
    function [F,dF] = diffStep_point(n_o,n_p,SVi,SVf,STMif,dSVf,dSVi)
        % - Constaint Vector
        F = SVf{n_o}(4:6)-SVi{n_p}(4:6); % Negetive of Delta-V
        % - Derivative Vector
        ...
        Mo  = (Dpo/Bpo)*Apo-Cpo;
        Mto = dSVf{n_o}(4:6) - (Dpo/Bpo)*SVf{n_o}(4:6);
        Mp  = Dpf/Bpf - Dpo/Bpo;
        Mtp = (Dpo/Bpo)*SVf{n_o}(4:6) - (Dpf/Bpf)*SVi{n_p}(4:6) ...
            + dSVi{n_p}(4:6) - dSVf{n_o}(4:6);
        Mf  = Cpf - (Dpf/Bpf)*Apf;
        Mtf = (Dpf/Bpf)*SVi{n_p}(4:6)-dSVi{n_p}(4:6);
