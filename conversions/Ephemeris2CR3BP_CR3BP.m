function [SV_cr3bp,t_cr3bp] = Ephemeris2CR3BP_CR3BP(SV_ephem,t_ephem,et0,primary,secondary,frame)
% =======================================================================
%          Ephemeris Frame to Non-dimensional CR3BP Rotational 
% =======================================================================
%
...
%
% Format : [SV_cr3bp,t_cr3bp] = Ephemeris2CR3BP_CR3BP(SV_ephem,t_ephem,...
%                       et0,primary,secondary,frame)
%
% -----------------------------------------------------------------------
%                               INPUTS
% -----------------------------------------------------------------------
...
for n = 1:N
    ...
    % -- Conversion to primary centered synodic frame
    SV_pc_rot = A_full' * SV_ephem(:,n);
    
    % -- Extracting primary centered position and velocity
    r_pc = SV_pc_rot(1:3);
    v_pc = SV_pc_rot(4:6);
    
    % -- Converting to non-dimesnional 
    r_pc_nd = r_pc/l_star;
    v_pc_nd = v_pc/v_star;
    
    % -- barrycenter position and velocity in rotating frame (non-dim)
    r_p = [mu;0;0];
    v_p = [0;0;0];
    
    % -- Converting to barrycenter non-dimensional
    r_rot_nd = r_pc_nd - r_p;
    v_rot_nd = v_pc_nd - v_p;
    
    % -- Adding to CR3BP array
    SV_cr3bp(1:3,n) = r_rot_nd;
    SV_cr3bp(4:6,n) = v_rot_nd;
    
    % -- Computing equivalent non-dimensional time
    t_cr3bp(n) = t_cr3bp_last + (t_ephem(n)-t_ephem_last)/t_star;
   
    et_last = et0 + t_ephem(n);
